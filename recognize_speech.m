function user_name = recognize_speech(audio_path)
	...
	user_names = ['Federico Romarion'; 'Federico Elli'; 'German Romarion'; 'Juana Unamuno'; 'Leonardo Rivas'; 'Mario Romarion'; 'Matias Rivas'; 'Milagros Rivas'; 'Sonia Rivas'; 'Stella Giunta'];

	for i = 1 : length(users)
		filename = strcat('audios/samples_1/a_la_grande_le_puse_cuca_', users{i})
		wavreads{i} = wavread(filename);
		codebooks{i} = vq(wavreads{i}, 16);
	end
	...
	for i = 1 : length(users)
		meandist = meandist(input_codebook, codebooks{i});
		if meandist < min_meandist
			min_meandist = meandist;
			min_codebook_index = i;
		end
	end

	user_name = user_names(min_codebook);
end